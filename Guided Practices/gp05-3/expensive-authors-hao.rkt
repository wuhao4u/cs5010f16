...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname expensive-authors-hao) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require rackunit)
(require "extras.rkt")
...
;; Using a HOF, write a function authors-of-expensive-books
;; that takes a price and a list of books,
;; and returns the list of all the authors of books
;; whose prices are greater than the given price.
(define (authors-of-expensive-books p lob)
  (filter (lambda (x) (> (book-price x) p)) lob))

(begin-for-test
  (check-equal? (authors-of-expensive-books 10 lob1) (list (make-book "Wand" "EOPL" 5 50))))